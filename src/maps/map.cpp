...
clustering DynamicalClustering_wrapper(at::Tensor &D, float eps, float lam)
{
...
clustering FDynamicalClustering_wrapper(at::Tensor &D, float eps, float lam, int B)
{
    return FDynamicalClustering(D.data_ptr<float>(), D.size(0), D.size(1), eps, lam, B);
}

//cuda wrapper
clustering GPU_DynamicalClustering_wrapper(at::Tensor &D, float eps, float lam)
{
...
clustering GPU_DynamicalClustering_GRID_STAD2_wrapper(at::Tensor &D, float eps, float lam, int cell_factor, int version)
{
...
clustering GPU_DynamicalClustering_DOUBLE_GRID_wrapper(at::Tensor &D, float eps, float lam, int cell_factor, int version)
{
    return GPU_DynamicalClustering_DOUBLE_GRID(D.data_ptr<float>(), D.size(0), D.size(1), eps, lam, cell_factor, version);
}
